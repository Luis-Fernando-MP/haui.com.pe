...
**haui.com.pe:** Prefer `Section`, region utilities, and shared chrome from the mandatory library in [haui.md](haui.md) (`Button`, `Title`, `Card`, …). Do not invent new section scaffolds when `Title` + `Section` fit; never invent local control twins.
