...
2. **haui.com.pe:** always Read [reference/haui.md](reference/haui.md) when this skill runs in this repository. Also apply `.agents/skills/coding-preferences/SKILL.md` on every code edit. Non-negotiable: clean code (no narrative comments, no duplication), **mandatory standard components** from `src/common/components` (see haui.md inventory: `Button`, `Card`, `CardTabs`, `Chip`, `DateFormat`, `Dialog`, `FocusGallery`, `Footer`, `Image`, `Input`, `MDX`, `Navbar`, `Popover`, `Popup`, `Select`, `Title` — never parallel local twins), type roles + semantic HTML, performance-aware, conditional UI with boolean `&&` — **never** JSX ternaries for show/hide. Themes in `themes.css`, type scale in `globals.css`. Honor root `AGENTS.md`.
3. Before acting, load the one playbook that owns the request: the Commands table's reference for an explicit or clearly implied sub-command, or [reference/new-work.md](reference/new-work.md) for a new surface or replacement visual world. Then inspect the target and at least one representative source of incumbent visual truth (tokens, theme, CSS, component, or asset) before editing.
4. After analysis and direction are resolved, load [reference/craft-floor.md](reference/craft-floor.md) immediately before editing UI. It carries the quality floor, the absolute bans, and the reflexes no detector catches. Do not load it for planning-only work. **Pinned haui brief wins** over generic craft-floor items that conflict (e.g. intentional mono `.type-label` section markers).
