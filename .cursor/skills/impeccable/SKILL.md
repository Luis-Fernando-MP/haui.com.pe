...
2. **haui.com.pe:** always Read [reference/haui.md](reference/haui.md) when this skill runs in this repository. Also apply `.agents/skills/coding-preferences/SKILL.md` on every code edit. Non-negotiable: clean code (no narrative comments, no duplication), standard components + type roles + semantic HTML, performance-aware, conditional UI with boolean `&&` — **never** JSX ternaries for show/hide. Reuse `src/common/components/*`, themes in `themes.css`, type scale in `globals.css`. Honor root `AGENTS.md`.
