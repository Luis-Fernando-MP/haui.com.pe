...
import { useFocusGalleryStore } from './store'
...
export const useFocusGallery = () => {
  const storeImages = useFocusGalleryStore(s => s.images)
  const storeIndex = useFocusGalleryStore(s => s.index)
  const closeStore = useFocusGalleryStore(s => s.close)
  ...
  useEffect(() => {
    ...
    setImageGallery(storeImages)
    setCurrentImageIndex(storeIndex)
  }, [storeImages, storeIndex])

  const isModalOpen = imageGallery.length > 0
  ...
  // Función para centrar la miniatura activa
  const scrollToActiveThumbnail = useCallback(() => {
    if (!thumbnailsContainerRef.current || !hasMultipleImages) return
    ...
    // Calcular la posición para centrar la miniatura activa
  ...
  }, [resetZoomAndPanState, closeStore])
  ...
  // Efecto para centrar la miniatura activa cuando cambia el índice
  useEffect(() => {
  ...
  }, [currentImageIndex, scrollToActiveThumbnail])
